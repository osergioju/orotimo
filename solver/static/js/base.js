...
function montarArray() {
    ...
    var encontrouErro = false; // Variável de controle
    ...
    formElements.forEach(function(formElement) {
        ...
        if (idMaterias.length == 0) {
            $('#receive_error').html('Preencha todos os campos');
            encontrouErro = true; // Marca que houve um erro
            return;
        }
    ...
    console.log(grupos_dados);

    var fakeclick = document.getElementById('fakeclick');
    // Só aciona o fakeclick se não encontrou erro
    if (!encontrouErro && d.value != '') {
        $('.carouseltype2').removeClass('error_professor');
        $('#receive_error').html('');
        fakeclick.click();
    }
}

$('.duplicater_add_div').each(function(){
    var qnt_divs = $(this).children('.relacional_professorxmateria').length;
    if(qnt_divs <= 0){
        $(this).closest('.carouseltype2').addClass('error_professor');
    }
});
